...
## Step 8
The code we have now will only get the agent down a row. The agent needs to turn left, move forward by 1 and then turn left again to be ready to move down the next row.  This needs to happen while still inside the first for loop. 
...
## Step 10
 Lastly, you need to make the agent turn right, move forward by 1, and then turn right again. This code needs to be inside the outer for loop. Close the outer for loop and the on chat command. 
