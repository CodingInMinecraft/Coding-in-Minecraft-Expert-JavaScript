# Expert Coding using JavaScript - Lesson 6 Activity 6.4
### @explicitHints true
...
All variables should be assigned the value of 0 EXCEPT width should be assigned the value of 15. 
...
## Step 3
Inside the second while loop, have the agent move forward by 1. Then increase the variable length by 1. 
After that, create an if / else if / else statement (with 3 else if sections).  In the If section, check that if the agent inspects a block forward and it is equal to a gold block do the following:
...
## Step 7
Then, in the else section, have the player say "No blocks!" Then close the if statement. Also close the inner while loop. 
...
## Step 9
Lastly, after the second inner while loop do the following: 
 have the variable width decrease by 1
...
Then close the outer while loop and close the on chat command. 
